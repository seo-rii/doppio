...
work_dir="${KOTLIN_MUTABLE_DELEGATE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-mutable-delegate-smoke"}"
...
source_dir="$repo_root/classes/kotlin_mutable_delegate_smoke"
...
compile_timeout="${KOTLIN_MUTABLE_DELEGATE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_MUTABLE_DELEGATE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_MUTABLE_DELEGATE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinMutableDelegateHelloKt.class"
...
expected_output="${KOTLIN_MUTABLE_DELEGATE_SMOKE_EXPECTED_OUTPUT:-"bind:primary:MutableDelegateOwner:primary:0|bind:primary:MutableDelegateOwner:primary:30|alt:secondary:MutableDelegateOwner:secondary:30|local:local:top:local:0|local:local:top:local:10"}"

native_output="$(java -cp "$runtime_cp" KotlinMutableDelegateHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinMutableDelegateHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin mutable delegate smoke passed in $((compile_end - compile_start))s."
