...
work_dir="${SCALA_LIBRARY_SMOKE_WORK_DIR:-"$repo_root/build/scala-library-smoke"}"
source_dir="$repo_root/classes/scala_library_smoke"
...
compile_timeout="${SCALA_LIBRARY_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_LIBRARY_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_LIBRARY_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaLibraryHello.class"
test -f "$out_dir/ScalaLibrarySmoke.class"
test -f "$out_dir/ScalaLibrarySmoke\$.class"
test -f "$out_dir/ScalaLibrarySmoke\$RichMetric.class"

expected_output="1=4,2=2,3=4:b:4/ccc:4/aa:2:g2:t5:String:3:z:2"

native_output="$(java -cp "$runtime_cp" ScalaLibraryHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaLibraryHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala library smoke passed in $((compile_end - compile_start))s using Scala $version."
