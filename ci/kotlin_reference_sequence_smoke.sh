...
work_dir="${KOTLIN_REFERENCE_SEQUENCE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-reference-sequence-smoke"}"
...
source_dir="$repo_root/classes/kotlin_reference_sequence_smoke"
...
compile_timeout="${KOTLIN_REFERENCE_SEQUENCE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_REFERENCE_SEQUENCE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_REFERENCE_SEQUENCE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinReferenceSequenceHelloKt.class"
...
expected_output="${KOTLIN_REFERENCE_SEQUENCE_SMOKE_EXPECTED_OUTPUT:-"a2|b7|c4|d9:20:8:7:10"}"

native_output="$(java -cp "$runtime_cp" KotlinReferenceSequenceHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinReferenceSequenceHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin reference/sequence smoke passed in $((compile_end - compile_start))s."
