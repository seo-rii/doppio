...
compile_timeout="${KOTLIN_CONTROL_FLOW_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_CONTROL_FLOW_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_CONTROL_FLOW_SMOKE_KILL_AFTER_SECONDS:-30}"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinControlFlowHelloKt)"
