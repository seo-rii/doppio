...
work_dir="${KOTLIN_COLLECTION_BUILDER_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-collection-builder-smoke"}"
...
source_dir="$repo_root/classes/kotlin_collection_builder_smoke"
...
compile_timeout="${KOTLIN_COLLECTION_BUILDER_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_COLLECTION_BUILDER_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_COLLECTION_BUILDER_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinCollectionBuilderHelloKt.class"
...
expected_output="${KOTLIN_COLLECTION_BUILDER_SMOKE_EXPECTED_OUTPUT:-"24678|k0=4,k1=16,k2=36,k3=49,k4=64|e=20,o=7|12,21,8|24|67|8|678/24|2:4;4:6;6:7|abc|22,44,66,77,88|1:3:7:13:20:28|71|2,1,0,9"}"

native_output="$(java -cp "$runtime_cp" KotlinCollectionBuilderHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinCollectionBuilderHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin collection builder smoke passed in $((compile_end - compile_start))s."
