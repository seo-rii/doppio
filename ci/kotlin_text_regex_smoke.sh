...
compile_timeout="${KOTLIN_TEXT_REGEX_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_TEXT_REGEX_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_TEXT_REGEX_SMOKE_KILL_AFTER_SECONDS:-30}"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinTextRegexHelloKt)"
