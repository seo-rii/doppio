...
work_dir="${KOTLIN_REIFIED_ARRAY_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-reified-array-smoke"}"
...
source_dir="$repo_root/classes/kotlin_reified_array_smoke"
...
compile_timeout="${KOTLIN_REIFIED_ARRAY_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_REIFIED_ARRAY_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_REIFIED_ARRAY_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinReifiedArrayHelloKt.class"
test -f "$out_dir/ReifiedArraySmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_REIFIED_ARRAY_SMOKE_EXPECTED_OUTPUT:-"String:3:a|bb|ccc:Number:2:1|2:i[3,1,4,9,1,5]=23:zamm|zbbmm:1-4-9:2345:String:int"}"

native_output="$(java -cp "$runtime_cp" KotlinReifiedArrayHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinReifiedArrayHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin reified/array smoke passed in $((compile_end - compile_start))s."
