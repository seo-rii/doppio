...
work_dir="${KOTLIN_CONTRACT_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-contract-smoke"}"
...
source_dir="$repo_root/classes/kotlin_contract_smoke"
...
compile_timeout="${KOTLIN_CONTRACT_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_CONTRACT_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_CONTRACT_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinContractHelloKt.class"
test -f "$out_dir/ContractSmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_CONTRACT_SMOKE_EXPECTED_OUTPUT:-"KT2|missing|before>body7>after|7|1002"}"

native_output="$(java -cp "$runtime_cp" KotlinContractHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinContractHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin contract smoke passed in $((compile_end - compile_start))s."
