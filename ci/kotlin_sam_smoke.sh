...
work_dir="${KOTLIN_SAM_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-sam-smoke"}"
...
source_dir="$repo_root/classes/kotlin_sam_smoke"
...
compile_timeout="${KOTLIN_SAM_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_SAM_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_SAM_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinSamHelloKt.class"
test -f "$out_dir/SamSmokeKt.class"
test -f "$out_dir/SamLabeler.class"
test -f "$out_dir/SamBox.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
bytecode_dump="$work_dir/kotlin-sam-javap.txt"
javap -classpath "$runtime_cp" -v SamSmokeKt > "$bytecode_dump"
grep -Fq 'InvokeDynamic' "$bytecode_dump"
grep -Fq 'java/lang/invoke/LambdaMetafactory.metafactory' "$bytecode_dump"
grep -Fq 'Ljava/lang/Runnable;' "$bytecode_dump"
grep -Fq 'Ljava/util/Comparator;' "$bytecode_dump"
grep -Fq 'Ljava/util/function/IntUnaryOperator;' "$bytecode_dump"
grep -Fq 'LSamLabeler;' "$bytecode_dump"

expected_output="${KOTLIN_SAM_SMOKE_EXPECTED_OUTPUT:-"L14|run|abbcc|M6|bbcc"}"

native_output="$(java -cp "$runtime_cp" KotlinSamHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinSamHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin SAM smoke passed in $((compile_end - compile_start))s."
