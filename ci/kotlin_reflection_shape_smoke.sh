...
work_dir="${KOTLIN_REFLECTION_SHAPE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-reflection-shape-smoke"}"
...
source_dir="$repo_root/classes/kotlin_reflection_shape_smoke"
...
compile_timeout="${KOTLIN_REFLECTION_SHAPE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_REFLECTION_SHAPE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_REFLECTION_SHAPE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinReflectionShapeHelloKt.class"
...
test -f "$out_dir/ReflectionShapeOwner\$anonymousRunnableClass\$1.class"
test -f "$out_dir/ReflectionShapeOwner\$methodLocalClass\$MethodLocal.class"
test -f "$out_dir/ReflectionShapeOwner\$Companion\$companionLocalClass\$CompanionLocal.class"
test -f "$out_dir/ReflectionShapeSmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_REFLECTION_SHAPE_SMOKE_EXPECTED_OUTPUT:-"Companion,Inner,Nested|Nested/ReflectionShapeOwner/ReflectionShapeOwner/true/false/false|Inner/ReflectionShapeOwner/ReflectionShapeOwner/true/false/false|Companion/ReflectionShapeOwner/ReflectionShapeOwner/true/false/false|MethodLocal/null/ReflectionShapeOwner/methodLocalClass/false/true/false|CompanionLocal/null/Companion/companionLocalClass/false/true/false|_/null/ReflectionShapeOwner/anonymousRunnableClass/Runnable/false/false/true"}"

native_output="$(java -cp "$runtime_cp" KotlinReflectionShapeHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinReflectionShapeHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin reflection shape smoke passed in $((compile_end - compile_start))s."
