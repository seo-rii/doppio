...
work_dir="${SCALA_INTEROP_SMOKE_WORK_DIR:-"$repo_root/build/scala-interop-smoke"}"
source_dir="$repo_root/classes/scala_interop_smoke"
...
compile_timeout="${SCALA_INTEROP_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_INTEROP_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_INTEROP_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaInteropHello.class"
test -f "$out_dir/ScalaInteropSmoke.class"
test -f "$out_dir/ScalaInteropSmoke\$.class"

scala_indy_dump="$work_dir/scala-interop-indy-javap.txt"
javap -classpath "$runtime_cp" -v 'ScalaInteropSmoke$' > "$scala_indy_dump"
grep -q 'InvokeDynamic' "$scala_indy_dump"
grep -q 'LambdaMetafactory' "$scala_indy_dump"

expected_output="r0a1b2:2:2,1:5:f32"

native_output="$(java -cp "$runtime_cp" ScalaInteropHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaInteropHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala interop smoke passed in $((compile_end - compile_start))s using Scala $version."
