...
work_dir="${KOTLIN_METHODHANDLE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-methodhandle-smoke"}"
compiler_jar="${KOTLIN_COMPILER_JAR:-}"
stdlib_jar="${KOTLIN_STDLIB_JAR:-}"
classpath_mode="${KOTLIN_METHODHANDLE_SMOKE_CLASSPATH_MODE:-minimal}"
compiler_cp="${KOTLIN_METHODHANDLE_COMPILER_CLASSPATH:-}"
...
if [ -z "$compiler_cp" ]; then
  case "$classpath_mode" in
    ...
    full)
      ...
      ;;
    *)
      echo "Invalid KOTLIN_METHODHANDLE_SMOKE_CLASSPATH_MODE: $classpath_mode" >&2
      exit 1
      ;;
  esac
fi
...
test -f "$out_dir/MethodHandleHelloKt.class"
...
echo "Kotlin MethodHandles smoke passed in $((compile_end - compile_start))s using $classpath_mode classpath."
