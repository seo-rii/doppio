...
work_dir="${KOTLIN_INITIALIZATION_DELEGATE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-initialization-delegate-smoke"}"
...
source_dir="$repo_root/classes/kotlin_initialization_delegate_smoke"
...
compile_timeout="${KOTLIN_INITIALIZATION_DELEGATE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_INITIALIZATION_DELEGATE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_INITIALIZATION_DELEGATE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinInitializationDelegateHelloKt.class"
...
expected_output="${KOTLIN_INITIALIZATION_DELEGATE_SMOKE_EXPECTED_OUTPUT:-"false/true|KT:5:1|KT:5:1|kt|2/9/6|companion>observed:start->kt>guarded:2?1>guarded:2?9>lazy>nested"}"

native_output="$(java -cp "$runtime_cp" KotlinInitializationDelegateHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinInitializationDelegateHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin initialization/delegate smoke passed in $((compile_end - compile_start))s."
