...
work_dir="${KOTLIN_EXTENSION_VARIANCE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-extension-variance-smoke"}"
...
source_dir="$repo_root/classes/kotlin_extension_variance_smoke"
...
compile_timeout="${KOTLIN_EXTENSION_VARIANCE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_EXTENSION_VARIANCE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_EXTENSION_VARIANCE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinExtensionVarianceHelloKt.class"
...
expected_output="${KOTLIN_EXTENSION_VARIANCE_SMOKE_EXPECTED_OUTPUT:-"5/1/5:1|5|3|p0:b:1,p1:aa:2|kt:2|ktxy|1,2|kt:2|xy:2|n:Integer,s:String,z:null"}"

native_output="$(java -cp "$runtime_cp" KotlinExtensionVarianceHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinExtensionVarianceHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin extension variance smoke passed in $((compile_end - compile_start))s."
