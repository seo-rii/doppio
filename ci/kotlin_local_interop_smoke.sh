...
cache_dir="${KOTLIN_LOCAL_INTEROP_SMOKE_CACHE_DIR:-"$repo_root/build/kotlin-smoke-cache"}"
work_dir="${KOTLIN_LOCAL_INTEROP_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-local-interop-smoke"}"
compiler_jar="${KOTLIN_COMPILER_JAR:-}"
stdlib_jar="${KOTLIN_STDLIB_JAR:-}"
compiler_cp="${KOTLIN_LOCAL_INTEROP_SMOKE_COMPILER_CLASSPATH:-}"
...
source_dir="${KOTLIN_LOCAL_INTEROP_SMOKE_SOURCE_DIR:-"$repo_root/classes/kotlin_local_interop_smoke"}"
...
compile_timeout="${KOTLIN_LOCAL_INTEROP_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_LOCAL_INTEROP_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_LOCAL_INTEROP_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinLocalInteropHelloKt.class"
test -f "$out_dir/InteropKt.class"
test -f "$out_dir/OuterSmoke.class"
test -f "$out_dir/OuterSmoke\$Nested.class"
test -f "$out_dir/OuterSmoke\$Inner.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_LOCAL_INTEROP_SMOKE_EXPECTED_OUTPUT:-"OK:FALLBACK:3:9:2:1:accbbb:4:4:7"}"

native_output="$(java -cp "$runtime_cp" KotlinLocalInteropHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinLocalInteropHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin local interop smoke passed in $((compile_end - compile_start))s."
