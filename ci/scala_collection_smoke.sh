...
work_dir="${SCALA_COLLECTION_SMOKE_WORK_DIR:-"$repo_root/build/scala-collection-smoke"}"
source_dir="$repo_root/classes/scala_collection_smoke"
...
compile_timeout="${SCALA_COLLECTION_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_COLLECTION_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_COLLECTION_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaCollectionHello.class"
test -f "$out_dir/ScalaCollectionSmoke.class"
test -f "$out_dir/ScalaCollectionSmoke\$.class"

expected_output="1,3,5,7:1=8,2=5:a5|z2:134:k10:r6"

native_output="$(java -cp "$runtime_cp" ScalaCollectionHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaCollectionHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala collection smoke passed in $((compile_end - compile_start))s using Scala $version."
