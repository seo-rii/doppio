...
work_dir="${KOTLIN_VALUE_CLASS_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-value-class-smoke"}"
...
source_dir="$repo_root/classes/kotlin_value_class_smoke"
...
compile_timeout="${KOTLIN_VALUE_CLASS_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_VALUE_CLASS_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_VALUE_CLASS_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinValueClassHelloKt.class"
...
expected_output="${KOTLIN_VALUE_CLASS_SMOKE_EXPECTED_OUTPUT:-"v1,v4,v7:22:box4:v7:none|v11:a"}"

native_output="$(java -cp "$runtime_cp" KotlinValueClassHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinValueClassHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin value-class smoke passed in $((compile_end - compile_start))s."
