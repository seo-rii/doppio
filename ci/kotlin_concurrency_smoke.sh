...
work_dir="${KOTLIN_CONCURRENCY_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-concurrency-smoke"}"
...
source_dir="$repo_root/classes/kotlin_concurrency_smoke"
...
compile_timeout="${KOTLIN_CONCURRENCY_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_CONCURRENCY_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_CONCURRENCY_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinConcurrencyHelloKt.class"
test -f "$out_dir/ConcurrentCacheSmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_CONCURRENCY_SMOKE_EXPECTED_OUTPUT:-"a=123,b=12,c=89|true|true:y:Y11|abc:false:true:1|main:11/worker:3/main:11/main:11|locked:3:hold:1:true|k=1,z=12|11"}"

native_output="$(java -cp "$runtime_cp" KotlinConcurrencyHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinConcurrencyHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin concurrency smoke passed in $((compile_end - compile_start))s."
