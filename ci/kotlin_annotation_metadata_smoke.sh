...
work_dir="${KOTLIN_ANNOTATION_METADATA_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-annotation-metadata-smoke"}"
...
source_dir="$repo_root/classes/kotlin_annotation_metadata_smoke"
...
compile_timeout="${KOTLIN_ANNOTATION_METADATA_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_ANNOTATION_METADATA_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_ANNOTATION_METADATA_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinAnnotationMetadataHelloKt.class"
...
expected_output="${KOTLIN_ANNOTATION_METADATA_SMOKE_EXPECTED_OUTPUT:-"class-a,class-b|class:HIGH:AnnotationMetadataOwner:1,2,3|ctor-a,ctor-b|ctor:LOW:String:4,5|field-a,field-b|field:LOW:int:6|method-a,method-b|method:LOW:long:7,8|arg-a,arg-b|arg:LOW:double:9|kt3"}"

native_output="$(java -cp "$runtime_cp" KotlinAnnotationMetadataHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinAnnotationMetadataHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin annotation metadata smoke passed in $((compile_end - compile_start))s."
