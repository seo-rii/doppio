...
work_dir="${SCALA_PACKAGE_SMOKE_WORK_DIR:-"$repo_root/build/scala-package-smoke"}"
source_dir="$repo_root/classes/scala_package_smoke"
...
compile_timeout="${SCALA_PACKAGE_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_PACKAGE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_PACKAGE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaPackageHello.class"
test -f "$out_dir/scalasmoke/NamedThing.class"
...
scala_signature_dump="$work_dir/scala-package-signature-javap.txt"
javap -classpath "$runtime_cp" -v scalasmoke.SpecializedBox > "$scala_signature_dump"
grep -Fq '// (TA;)Lscala/Tuple2<TA;TA;>;' "$scala_signature_dump"

expected_output="worker:3:c1:describe/getName/total:pkg-worker-11:20:red-green-blue:23"

native_output="$(java -cp "$runtime_cp" ScalaPackageHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaPackageHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala package smoke passed in $((compile_end - compile_start))s using Scala $version."
