...
work_dir="${SCALA_REFLECT_SMOKE_WORK_DIR:-"$repo_root/build/scala-reflect-smoke"}"
source_dir="$repo_root/classes/scala_reflect_smoke"
...
runtime_cp="$out_dir:$library_jar:$reflect_jar"
...
compile_timeout="${SCALA_REFLECT_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_REFLECT_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_REFLECT_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaReflectHello.class"
test -f "$out_dir/ScalaReflectSmoke.class"
test -f "$out_dir/ScalaReflectSmoke\$.class"
test -f "$out_dir/ScalaReflectSmoke\$ReflectBox.class"

expected_output="ReflectBox:2:name/value:true"

native_output="$(java -cp "$runtime_cp" ScalaReflectHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaReflectHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala runtime reflection smoke passed in $((compile_end - compile_start))s using Scala $version."
