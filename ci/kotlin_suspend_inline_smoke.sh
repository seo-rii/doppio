...
work_dir="${KOTLIN_SUSPEND_INLINE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-suspend-inline-smoke"}"
...
source_dir="$repo_root/classes/kotlin_suspend_inline_smoke"
...
compile_timeout="${KOTLIN_SUSPEND_INLINE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_SUSPEND_INLINE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_SUSPEND_INLINE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinSuspendInlineHelloKt.class"
test -f "$out_dir/SuspendInlineFinallyKt.class"
test -f "$out_dir/SuspendInlineFinallyKt\$runSuspendInline\$1.class"
test -f "$out_dir/SuspendInlineFinallyKt\$runSuspendInline\$block\$1.class"
test -f "$out_dir/META-INF/main.kotlin_module"

inline_state_class="$out_dir/SuspendInlineFinallyKt\$runSuspendInline\$block\$1.class"
inline_state_javap="$(javap -c -p "$inline_state_class")"
for marker in \
    'tableswitch' \
    'SuspendInlineFinallyKt.access$inlineCheckpoint' \
    'String enter' \
    'String exit' \
    'Exception table:'; do
  if ! grep -Fq "$marker" <<<"$inline_state_javap"; then
    echo "Missing suspend-inline state-machine marker: $marker" >&2
    exit 1
  fi
done
if grep -Fq 'tracedInline' <<<"$inline_state_javap"; then
  echo "Suspend-inline state machine retained a tracedInline call." >&2
  exit 1
fi

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_SUSPEND_INLINE_SMOKE_EXPECTED_OUTPUT:-"pending>done5>enter>body>wait>after:4>exit|pending>fail:boom>enter>body>wait>exit"}"

native_output="$(java -cp "$runtime_cp" KotlinSuspendInlineHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinSuspendInlineHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin suspend-inline smoke passed in $((compile_end - compile_start))s."
