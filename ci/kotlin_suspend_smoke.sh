...
work_dir="${KOTLIN_SUSPEND_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-suspend-smoke"}"
compiler_jar="${KOTLIN_COMPILER_JAR:-}"
stdlib_jar="${KOTLIN_STDLIB_JAR:-}"
classpath_mode="${KOTLIN_SUSPEND_SMOKE_CLASSPATH_MODE:-minimal}"
compiler_cp="${KOTLIN_SUSPEND_COMPILER_CLASSPATH:-}"
...
if [ -z "$compiler_cp" ]; then
  case "$classpath_mode" in
    ...
    full)
      ...
      ;;
    *)
      echo "Invalid KOTLIN_SUSPEND_SMOKE_CLASSPATH_MODE: $classpath_mode" >&2
      exit 1
      ;;
  esac
fi
...
source_dir="$repo_root/classes/kotlin_suspend_smoke"
...
compile_timeout="${KOTLIN_SUSPEND_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_SUSPEND_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_SUSPEND_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/SuspendHelloKt.class"
test -f "$out_dir/SuspendSmokeKt.class"
test -f "$out_dir/SmokeQueueDispatcher.class"
test -f "$out_dir/SmokeQueueDispatcher\$interceptContinuation\$1.class"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="$(printf 'suspend=7\nstate=14\npending->delayed=15\npending->fail=resume3\npending->thread=24\npending->executor=13\npending>1>pending>pending>1>pending>pending>1>dispatch=36')"

native_output="$(java -cp "$runtime_cp" SuspendHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" SuspendHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin suspend smoke passed in $((compile_end - compile_start))s using $classpath_mode classpath."
