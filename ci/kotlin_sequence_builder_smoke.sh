...
work_dir="${KOTLIN_SEQUENCE_BUILDER_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-sequence-builder-smoke"}"
...
source_dir="$repo_root/classes/kotlin_sequence_builder_smoke"
...
compile_timeout="${KOTLIN_SEQUENCE_BUILDER_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_SEQUENCE_BUILDER_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_SEQUENCE_BUILDER_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinSequenceBuilderHelloKt.class"
...
expected_output="${KOTLIN_SEQUENCE_BUILDER_SMOKE_EXPECTED_OUTPUT:-"1:2|start>after1|0=3,1=8,2=10|start>after1>start>after1>afterAll>done|abcd|789/IllegalStateException|3,6,12,24|xyyzzz:23"}"

native_output="$(java -cp "$runtime_cp" KotlinSequenceBuilderHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinSequenceBuilderHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin sequence builder smoke passed in $((compile_end - compile_start))s."
