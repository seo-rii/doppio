...
work_dir="${KOTLIN_BYTECODE_RUNTIME_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-bytecode-runtime-smoke"}"
...
source_dir="$repo_root/classes/kotlin_bytecode_runtime_smoke"
out_dir="$work_dir/out"

rm -rf "$out_dir"
mkdir -p "$out_dir"

compile_timeout="${KOTLIN_BYTECODE_RUNTIME_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_BYTECODE_RUNTIME_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
...
test -f "$out_dir/KotlinBytecodeRuntimeHelloKt.class"
...
expected_output="${KOTLIN_BYTECODE_RUNTIME_SMOKE_EXPECTED_OUTPUT:-"try>catch>finally:boom:8:true:x3:10:12:4:sync"}"
...
doppio_output="$(timeout -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinBytecodeRuntimeHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin bytecode runtime smoke passed in $((compile_end - compile_start))s."
