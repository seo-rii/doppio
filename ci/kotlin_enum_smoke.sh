...
work_dir="${KOTLIN_ENUM_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-enum-smoke"}"
...
source_dir="$repo_root/classes/kotlin_enum_smoke"
...
compile_timeout="${KOTLIN_ENUM_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_ENUM_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_ENUM_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinEnumHelloKt.class"
...
expected_output="${KOTLIN_ENUM_SMOKE_EXPECTED_OUTPUT:-"A1B3G5|tk2|X1,x2,xxx3|low/high/high|IllegalArgumentException"}"

native_output="$(java -cp "$runtime_cp" KotlinEnumHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinEnumHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin enum smoke passed in $((compile_end - compile_start))s."
