...
work_dir="${SCALA_REFLECTION_SHAPE_SMOKE_WORK_DIR:-"$repo_root/build/scala-reflection-shape-smoke"}"
source_dir="$repo_root/classes/scala_reflection_shape_smoke"
...
compile_timeout="${SCALA_REFLECTION_SHAPE_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_REFLECTION_SHAPE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_REFLECTION_SHAPE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaReflectionShapeHello.class"
...
expected_output="member/Member/ScalaReflectionShapeOwner/ScalaReflectionShapeOwner/null/-/true/false/false|local/MethodLocal\$1/null/ScalaReflectionShapeOwner/methodLocalClass/-/false/true/false|anonymous/_/null/ScalaReflectionShapeOwner/anonymousRunnableClass/Runnable/false/false/true"

native_output="$(java -cp "$runtime_cp" ScalaReflectionShapeHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaReflectionShapeHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala reflection-shape smoke passed in $((compile_end - compile_start))s using Scala $version."
