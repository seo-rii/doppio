...
work_dir="${KOTLIN_CAPTURE_SHAPE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-capture-shape-smoke"}"
...
source_dir="$repo_root/classes/kotlin_capture_shape_smoke"
...
compile_timeout="${KOTLIN_CAPTURE_SHAPE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_CAPTURE_SHAPE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_CAPTURE_SHAPE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinCaptureShapeHelloKt.class"
...
expected_output="${KOTLIN_CAPTURE_SHAPE_SMOKE_EXPECTED_OUTPUT:-"234:yx:true:11:45|89:yx:true:8:5"}"

native_output="$(java -cp "$runtime_cp" KotlinCaptureShapeHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinCaptureShapeHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin capture shape smoke passed in $((compile_end - compile_start))s."
