...
cache_dir="${KOTLIN_BASIC_CONSTRUCT_SMOKE_CACHE_DIR:-"$repo_root/build/kotlin-smoke-cache"}"
work_dir="${KOTLIN_BASIC_CONSTRUCT_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-basic-construct-smoke"}"
compiler_jar="${KOTLIN_COMPILER_JAR:-}"
stdlib_jar="${KOTLIN_STDLIB_JAR:-}"
compiler_cp="${KOTLIN_BASIC_CONSTRUCT_SMOKE_COMPILER_CLASSPATH:-}"
...
source_dir="${KOTLIN_BASIC_CONSTRUCT_SMOKE_SOURCE_DIR:-"$repo_root/classes/kotlin_basic_construct_smoke"}"
...
compile_timeout="${KOTLIN_BASIC_CONSTRUCT_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_BASIC_CONSTRUCT_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_BASIC_CONSTRUCT_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinBasicConstructHelloKt.class"
test -f "$out_dir/ConstructsKt.class"
test -f "$out_dir/SmokePoint.class"
test -f "$out_dir/SmokeNamed.class"
test -f "$out_dir/SmokeBox.class"
test -f "$out_dir/SmokeTag.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_BASIC_CONSTRUCT_SMOKE_EXPECTED_OUTPUT:-"name=2,4:5"}"

native_output="$(java -cp "$runtime_cp" KotlinBasicConstructHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinBasicConstructHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin basic construct smoke passed in $((compile_end - compile_start))s."
