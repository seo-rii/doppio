...
kill_after="${SCALA_IO_SMOKE_KILL_AFTER_SECONDS:-30}"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaIoHello)"
