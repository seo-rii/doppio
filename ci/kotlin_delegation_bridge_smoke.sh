...
work_dir="${KOTLIN_DELEGATION_BRIDGE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-delegation-bridge-smoke"}"
...
source_dir="$repo_root/classes/kotlin_delegation_bridge_smoke"
...
compile_timeout="${KOTLIN_DELEGATION_BRIDGE_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_DELEGATION_BRIDGE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_DELEGATION_BRIDGE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinDelegationBridgeHelloKt.class"
...
expected_output="${KOTLIN_DELEGATION_BRIDGE_SMOKE_EXPECTED_OUTPUT:-"text:7|text:5|5x|text:6|z!|az!|apply:Object:Object,describe:String:CharSequence,describe:String:Object|apply:Object:Object,describe:String:Object|echo:Object:Object,read:Object:"}"

native_output="$(java -cp "$runtime_cp" KotlinDelegationBridgeHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinDelegationBridgeHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin delegation bridge smoke passed in $((compile_end - compile_start))s."
