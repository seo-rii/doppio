...
work_dir="${KOTLIN_INLINE_CONTROL_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-inline-control-smoke"}"
...
source_dir="$repo_root/classes/kotlin_inline_control_smoke"
...
compile_timeout="${KOTLIN_INLINE_CONTROL_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_INLINE_CONTROL_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_INLINE_CONTROL_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinInlineControlHelloKt.class"
...
expected_output="${KOTLIN_INLINE_CONTROL_SMOKE_EXPECTED_OUTPUT:-"enter>body>exit:ok:c10:34:stop3"}"

native_output="$(java -cp "$runtime_cp" KotlinInlineControlHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinInlineControlHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin inline-control smoke passed in $((compile_end - compile_start))s."
