...
work_dir="${KOTLIN_COROUTINE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-coroutine-smoke"}"
...
source_file="$repo_root/classes/kotlin_coroutine_smoke/CoroutineFinally.kt"
...
compile_timeout="${KOTLIN_COROUTINE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_COROUTINE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_COROUTINE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/CoroutineFinallyKt.class"
test -f "$out_dir/CoroutineFinallyKt\$main\$block\$1.class"
test -f "$out_dir/QueueDispatcher\$interceptContinuation\$1.class"
test -f "$out_dir/QueueDispatcher.class"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_COROUTINE_SMOKE_EXPECTED_OUTPUT:-"pending|1|pending|pending|1|pending|pending|1|result=27|clean>inner>outer"}"

native_output="$(java -cp "$runtime_cp" CoroutineFinallyKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" CoroutineFinallyKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin coroutine smoke passed in $((compile_end - compile_start))s."
