...
work_dir="${KOTLIN_MODERN_CONSTRUCT_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-modern-construct-smoke"}"
...
source_dir="$repo_root/classes/kotlin_modern_construct_smoke"
...
compile_timeout="${KOTLIN_MODERN_CONSTRUCT_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_MODERN_CONSTRUCT_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_MODERN_CONSTRUCT_SMOKE_KILL_AFTER_SECONDS:-30}"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinModernConstructHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin modern construct smoke passed in $((compile_end - compile_start))s."
