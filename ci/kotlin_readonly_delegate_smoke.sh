...
work_dir="${KOTLIN_READONLY_DELEGATE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-readonly-delegate-smoke"}"
...
source_dir="$repo_root/classes/kotlin_readonly_delegate_smoke"
...
compile_timeout="${KOTLIN_READONLY_DELEGATE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_READONLY_DELEGATE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_READONLY_DELEGATE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinReadonlyDelegateHelloKt.class"
test -f "$out_dir/DelegateSmokeKt.class"
test -f "$out_dir/DelegatedOwner.class"
test -f "$out_dir/PrefixDelegate.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_READONLY_DELEGATE_SMOKE_EXPECTED_OUTPUT:-"delegate:answer:DelegatedOwner|local:local:top"}"

native_output="$(java -cp "$runtime_cp" KotlinReadonlyDelegateHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinReadonlyDelegateHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin read-only delegate smoke passed in $((compile_end - compile_start))s."
