...
work_dir="${KOTLIN_COMPLETABLE_FUTURE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-completable-future-smoke"}"
...
source_dir="$repo_root/classes/kotlin_completable_future_smoke"
...
compile_timeout="${KOTLIN_COMPLETABLE_FUTURE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_COMPLETABLE_FUTURE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_COMPLETABLE_FUTURE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinCompletableFutureHelloKt.class"
test -f "$out_dir/CompletableFutureSmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_COMPLETABLE_FUTURE_SMOKE_EXPECTED_OUTPUT:-"KT2!:5:l"}"

native_output="$(java -cp "$runtime_cp" KotlinCompletableFutureHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinCompletableFutureHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin CompletableFuture smoke passed in $((compile_end - compile_start))s."
