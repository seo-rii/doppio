...
work_dir="${SCALA_METHODHANDLE_SMOKE_WORK_DIR:-"$repo_root/build/scala-methodhandle-smoke"}"
source_dir="$repo_root/classes/scala_methodhandle_smoke"
...
compile_timeout="${SCALA_METHODHANDLE_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_METHODHANDLE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_METHODHANDLE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaMethodHandleHello.class"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaMethodHandleHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala MethodHandles smoke passed in $((compile_end - compile_start))s using Scala $version."
