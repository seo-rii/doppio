...
work_dir="${KOTLIN_UNSIGNED_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-unsigned-smoke"}"
...
source_dir="$repo_root/classes/kotlin_unsigned_smoke"
...
compile_timeout="${KOTLIN_UNSIGNED_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_UNSIGNED_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_UNSIGNED_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinUnsignedHelloKt.class"
test -f "$out_dir/UnsignedSmokeKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_UNSIGNED_SMOKE_EXPECTED_OUTPUT:-"3:4:0fa0ff:2,9,18446744073709551615:4294967295,4:wrap:true:true"}"

native_output="$(java -cp "$runtime_cp" KotlinUnsignedHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinUnsignedHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin unsigned smoke passed in $((compile_end - compile_start))s."
