...
work_dir="${KOTLIN_JVM_INTEROP_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-jvm-interop-smoke"}"
...
source_dir="$repo_root/classes/kotlin_jvm_interop_smoke"
out_dir="$work_dir/out"

rm -rf "$out_dir"
mkdir -p "$out_dir"

compile_timeout="${KOTLIN_JVM_INTEROP_SMOKE_COMPILE_TIMEOUT_SECONDS:-360}"
run_timeout="${KOTLIN_JVM_INTEROP_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
...
test -f "$out_dir/KotlinJvmInteropHelloKt.class"
...
expected_output="${KOTLIN_JVM_INTEROP_SMOKE_EXPECTED_OUTPUT:-"kt:java:ok7:IllegalArgumentException:fieldconst:top-3:o5obj:5:11111111"}"
...
doppio_output="$(timeout -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinJvmInteropHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin JVM interop smoke passed in $((compile_end - compile_start))s."
