...
work_dir="${SCALA_PROXY_SMOKE_WORK_DIR:-"$repo_root/build/scala-proxy-smoke"}"
source_dir="$repo_root/classes/scala_proxy_smoke"
...
source_cp="$support_dir:$library_jar"
...
compile_timeout="${SCALA_PROXY_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_PROXY_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_PROXY_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaProxyHello.class"
test -f "$out_dir/ScalaProxyReflectionSmoke.class"
test -f "$out_dir/ScalaProxyService.class"

expected_output="iface:transform:value:dyn:SC5:XY3:cba:null:ScalaProxyService(dyn):654:true:true:true:transform:2,label:0,transform:2,maybe:1,maybe:1,toString:0,hashCode:0,equals:1"

native_output="$(java -cp "$runtime_cp" ScalaProxyHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaProxyHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala proxy smoke passed in $((compile_end - compile_start))s using Scala $version."
