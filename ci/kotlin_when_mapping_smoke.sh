...
work_dir="${KOTLIN_WHEN_MAPPING_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-when-mapping-smoke"}"
...
source_dir="$repo_root/classes/kotlin_when_mapping_smoke"
...
compile_timeout="${KOTLIN_WHEN_MAPPING_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_WHEN_MAPPING_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_WHEN_MAPPING_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinWhenMappingHelloKt.class"
...
expected_output="${KOTLIN_WHEN_MAPPING_SMOKE_EXPECTED_OUTPUT:-"1357:nilpe:14:10,30,-1,40:neg|zero|small|big"}"

native_output="$(java -cp "$runtime_cp" KotlinWhenMappingHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinWhenMappingHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin when-mapping smoke passed in $((compile_end - compile_start))s."
