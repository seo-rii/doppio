...
work_dir="${SCALA_STACKWALKER_SMOKE_WORK_DIR:-"$repo_root/build/scala-stackwalker-smoke"}"
source_dir="$repo_root/classes/scala_stackwalker_smoke"
...
compile_timeout="${SCALA_STACKWALKER_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_STACKWALKER_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${SCALA_STACKWALKER_SMOKE_KILL_AFTER_SECONDS:-30}"
...
doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaStackWalkerHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala StackWalker smoke passed in $((compile_end - compile_start))s using Scala $version."
