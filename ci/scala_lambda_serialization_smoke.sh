...
work_dir="${SCALA_LAMBDA_SERIALIZATION_SMOKE_WORK_DIR:-"$repo_root/build/scala-lambda-serialization-smoke"}"
source_dir="$repo_root/classes/scala_lambda_serialization_smoke"
...
compile_timeout="${SCALA_LAMBDA_SERIALIZATION_SMOKE_COMPILE_TIMEOUT_SECONDS:-420}"
run_timeout="${SCALA_LAMBDA_SERIALIZATION_SMOKE_RUN_TIMEOUT_SECONDS:-90}"
kill_after="${SCALA_LAMBDA_SERIALIZATION_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/ScalaLambdaSerializationHello.class"
test -f "$out_dir/ScalaLambdaSerializationSmoke.class"
test -f "$out_dir/ScalaLambdaSerializationSmoke\$.class"
test -f "$out_dir/SerializableLambdaFactory.class"

lambda_javap="$(javap -classpath "$runtime_cp" -v -p SerializableLambdaFactory)"
for marker in \
    'InvokeDynamic' \
    'LambdaMetafactory.altMetafactory' \
    '$deserializeLambda$' \
    'java/lang/invoke/SerializedLambda'; do
  if ! grep -Fq "$marker" <<<"$lambda_javap"; then
    echo "Missing serialized-lambda classfile marker: $marker" >&2
    exit 1
  fi
done

expected_output="12:12:15:true:true"

native_output="$(java -cp "$runtime_cp" ScalaLambdaSerializationHello)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" ScalaLambdaSerializationHello)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Scala lambda serialization smoke passed in $((compile_end - compile_start))s using Scala $version."
