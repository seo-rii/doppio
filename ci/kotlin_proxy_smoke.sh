...
work_dir="${KOTLIN_PROXY_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-proxy-smoke"}"
...
source_dir="$repo_root/classes/kotlin_proxy_smoke"
...
compile_timeout="${KOTLIN_PROXY_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_PROXY_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_PROXY_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/KotlinProxyHelloKt.class"
...
expected_output="${KOTLIN_PROXY_SMOKE_EXPECTED_OUTPUT:-"iface/transform/value|dyn|KT5|XY3|cba|null|ProxyReflectionService(dyn)|321|true|true|true|transform:2,getLabel:0,transform:2,maybe:1,maybe:1,toString:0,hashCode:0,equals:1"}"

native_output="$(java -cp "$runtime_cp" KotlinProxyHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" KotlinProxyHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin proxy smoke passed in $((compile_end - compile_start))s."
