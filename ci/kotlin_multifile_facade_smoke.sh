...
cache_dir="${KOTLIN_MULTIFILE_FACADE_SMOKE_CACHE_DIR:-"$repo_root/build/kotlin-smoke-cache"}"
work_dir="${KOTLIN_MULTIFILE_FACADE_SMOKE_WORK_DIR:-"$repo_root/build/kotlin-multifile-facade-smoke"}"
compiler_jar="${KOTLIN_COMPILER_JAR:-}"
stdlib_jar="${KOTLIN_STDLIB_JAR:-}"
compiler_cp="${KOTLIN_MULTIFILE_FACADE_SMOKE_COMPILER_CLASSPATH:-}"
...
source_dir="${KOTLIN_MULTIFILE_FACADE_SMOKE_SOURCE_DIR:-"$repo_root/classes/kotlin_multifile_facade_smoke"}"
...
compile_timeout="${KOTLIN_MULTIFILE_FACADE_SMOKE_COMPILE_TIMEOUT_SECONDS:-480}"
run_timeout="${KOTLIN_MULTIFILE_FACADE_SMOKE_RUN_TIMEOUT_SECONDS:-60}"
kill_after="${KOTLIN_MULTIFILE_FACADE_SMOKE_KILL_AFTER_SECONDS:-30}"
...
test -f "$out_dir/smoke/multifile/KotlinMultifileFacadeHelloKt.class"
test -f "$out_dir/smoke/multifile/FacadeSmoke.class"
test -f "$out_dir/smoke/multifile/FacadeSmoke__PartOneKt.class"
test -f "$out_dir/smoke/multifile/FacadeSmoke__PartTwoKt.class"
test -f "$out_dir/META-INF/main.kotlin_module"

runtime_cp="$out_dir:$stdlib_jar"
expected_output="${KOTLIN_MULTIFILE_FACADE_SMOKE_EXPECTED_OUTPUT:-"mf4:mf2.mf3:p8:FacadeSmoke:true"}"

native_output="$(java -cp "$runtime_cp" smoke.multifile.KotlinMultifileFacadeHelloKt)"
if [ "$native_output" != "$expected_output" ]; then
  echo "Unexpected native JVM output: $native_output" >&2
  exit 1
fi

doppio_output="$(timeout -k "${kill_after}s" -s INT "${run_timeout}s" node --no-deprecation "$runner" -cp "$runtime_cp" smoke.multifile.KotlinMultifileFacadeHelloKt)"
if [ "$doppio_output" != "$expected_output" ]; then
  echo "Unexpected Doppio output: $doppio_output" >&2
  exit 1
fi

echo "Kotlin multifile facade smoke passed in $((compile_end - compile_start))s."
