...
- The main smoke also includes reflection-backed Kotlin source calls into
  ...
  modern runtime overlays under both the host JVM and Doppio. It also directly
  calls Java 16 `Stream.toList()` and Java 17 `Map.Entry.copyOf` from Kotlin
  source and checks their unmodifiable-result behavior.
