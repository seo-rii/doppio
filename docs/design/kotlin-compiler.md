...
- The main smoke also includes reflection-backed Kotlin source calls into
  selected Java 17 class-library APIs that are not visible on the Java 8 boot
  surface: `java.util.HexFormat`, `java.time.InstantSource`, and
  `java.util.random.RandomGeneratorFactory`. This keeps the compiler classpath
  compatible while verifying that Kotlin-generated bytecode can consume those
  modern runtime overlays under both the host JVM and Doppio, including seeded
  bounded output for the `Random` and `SplittableRandom` providers. It also
  directly calls Java 16 `Stream.toList()` and Java 17 `Map.Entry.copyOf` from
  Kotlin source and checks their unmodifiable-result behavior.
