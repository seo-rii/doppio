...
| Method-handle combinators | Partial | Compose same-class method handles with selected JDK combinators | `identity`, `constant`, `bindTo`, `insertArguments`, `dropArguments`, `filterArguments`, `filterReturnValue`, `permuteArguments`, `guardWithTest`, `catchException`, `exactInvoker`, `invoker`, `spreadInvoker`, `collectArguments`, zero-position and selected nonzero-position `foldArguments`, `explicitCastArguments`, `arrayElementGetter`, `arrayElementSetter`, `throwException`, selected `MethodHandle.asCollector`, `asSpreader`, `asVarargsCollector`, and `asFixedArity` adapter flows, plus Java 17 public overlays `zero`, `empty`, `arrayLength`, `arrayConstructor`, `dropArgumentsToMatch`, `dropReturn`, selected `tryFinally`, `tableSwitch`, and `iteratedLoop` flows including `void` table-switch targets and no-arg explicit iterator loops, selected `whileLoop`/`doWhileLoop`/`countedLoop` non-void state plus void side-effect loops, and selected single-clause `loop` state flows produce native-compatible results and descriptor strings for the tested shapes | existing Java 8 method-handle adapter/combinator path plus class-load native overlay helpers |
...
- Java 17 `MethodHandles.Lookup` has a public same-class fixture covering
  ...
  covers `Lookup.unreflect`, `unreflectSpecial`, `unreflectConstructor`,
  `unreflectGetter`, and `unreflectSetter` for public members, selected
  superclass and interface-default special dispatch, private non-nestmate access
  failures, and private nestmate method, constructor, static field, and instance
  field success paths. It also covers selected `MethodHandles.reflectAs` round-trips
  from method, constructor, getter, and setter handles back to reflective
  members. `MethodHandles.privateLookupIn` is covered for same-package
  ...
  `spreadInvoker`, `collectArguments`, `foldArguments`,
  `explicitCastArguments`, `arrayElementGetter`, `arrayElementSetter`, and
  `throwException`, including descriptor-string checks for the adapted shapes.
  It also covers selected `MethodHandle.asCollector`, `asSpreader`,
  `asVarargsCollector`, and `asFixedArity` flows, including position overloads
  for trailing and middle argument groups. This does not yet claim every
...
- Java 12 nominal method-handle descriptors now resolve selected direct
  ...
  surface: public same-class static, virtual, special, and interface-special
  methods, constructors, static/instance field getters and setters, and
  `asType` wrappers. This is a class-library bridge over tested lookup paths,
  not a new general access model.
...
- `ConstantBootstraps.invoke` has targeted fast paths for selected method-handle
  ...
  arithmetic paths. Focused primitive-adaptation fixtures also feed a boxed
  `Long` dynamic constant into a `double` method parameter and a boxed
  `Integer` dynamic constant into a `float` method parameter, verifying
  unbox-then-widen argument conversion and float-width rounding. Static
...
- Java 17 public `MethodHandles` overlays that are absent from the Java 8
  ...
  `dropReturn`, selected nonzero-position `foldArguments`, selected
  ...
  plus selected `null step` and optional `fini`.
  ...
  `docs/design/methodhandles-control-flow.md`.
...
- The selected `whileLoop`, `doWhileLoop`, `countedLoop`, `iteratedLoop`, and
  single-clause `loop` slices cover non-`void` state loops plus selected
  ...
  validation errors. The selected `iteratedLoop` slice covers null-iterator
  ...
  `init`, selected explicit-init `null step`, and optional `fini`. Broad
  state-loop parity and the remaining multi-clause Java 9+ `loop` combinator
  still need separate fixtures and implementation slices because they need
  control-flow, exception, and argument-flow parity beyond the simple public
  overlay helper shape.
...
- The `ConstantBootstraps.invoke` fast path does not yet cover broad dispatch
  ...
  argument widening, boxed `Integer` to `float` argument widening,
  `CONSTANT_Integer` to
