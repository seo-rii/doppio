...
results. The full Java 9-26 suite passed locally on 2026-07-17 in 7 minutes 25
...
locally in 120 and 81 seconds respectively using only Doppio compiler metadata.
...
original `rt.jar` or generated `doppio.jar`. The generated compiler bootstrap
overlay now materializes the same transformed `Duration.class` and precedes
`doppio.jar` and `rt.jar` during source analysis. The smokes no longer use host
JDK class metadata; their generated bytecode still runs against the ordinary
single-transform Doppio runtime. See
