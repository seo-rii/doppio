...
`URLClassLoader`, opens the same archive through a Java 13
`FileSystems.newFileSystem(Path)` helper called from Scala-generated bytecode,
creates `META-INF/services` metadata for Scala-compiled provider classes,
checks duplicate-provider collapse and reload behavior, and verifies
class-relative, loader-relative, system, enumeration, stream, and Java 9
`ClassLoader.resources` lookup paths from Scala-compiled code while keeping the
main Scala compiler smoke smaller.
