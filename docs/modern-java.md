...
- `Class.getRecordComponents()` and a minimal
  ...
  non-generic components, accessor, declaring record, null for non-record
  classes, empty record arrays, and array cloning behavior. Full generic `Type`
  parsing, type-use annotations, and component annotation metadata are not
