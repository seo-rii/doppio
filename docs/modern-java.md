...
- The minimal `Hello.kt` smoke runs in the `Modern Java` GitHub Actions
  ...
  propagation, plus Java `Thread`-based continuation resumption,
  `try`/`catch`/`finally`, `Closeable.use`, destructuring, range loops, stepped
  `downTo` loops, `mapIndexed`, and `synchronized`, then runs the generated
  code on both the host JVM and Doppio with
  `kotlin-stdlib.jar`.
...
- `grunt --grunt-ignore-compile-errors test-modern-java` compiles and checks the
  ...
  multi-release JAR fixture whose `META-INF/versions/9` class entry must match
  native Java 17 selection, `StackWalker.getCallerClass`
  ...
  non-nestmate private failure paths, selected `MethodHandle.asType` reference
  cast, return widening-to-`Object`, non-void return dropping-to-`void`,
  `void` return-to-`null` reference adaptation, primitive argument and return
  widening, primitive return boxing, `Integer`-to-`int` unboxing, arity
  mismatch, and runtime cast failure paths, sealed metadata, and
...
- `Class.getRecordComponents()`, `java.lang.reflect.RecordComponent`, and
  component annotation metadata are not implemented. See
  `docs/design/record-reflection.md` for the Class/RecordComponent design split
  needed before implementing this safely.
