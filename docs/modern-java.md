...
- `java.lang.management.ManagementFactory` now covers selected runtime,
  ...
  memory verbose no-op behavior, and synthetic heap/non-heap `MemoryUsage`
  values with stable field invariants.
- The shim intentionally reports synthetic in-VM counts and empty VM input
  ...
  memory accounting, detailed HotSpot compilation counters,
