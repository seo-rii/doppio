...
- `Class.getRecordComponents()` and a minimal
  `java.lang.reflect.RecordComponent` shim now cover component name, type,
  accessor, declaring record, null for non-record classes, empty record arrays,
  and array cloning behavior. Component generic-signature and annotation
  metadata are not implemented. See `docs/design/record-reflection.md` for the
  remaining Class/RecordComponent design split before expanding this surface.
