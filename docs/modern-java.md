...
- Direct and reflective calls to every Java 18 `Math` and `StrictMath`
  ...
  parity. Parsed public static methods injected into both bootstrap classes
  share one `DoppioMath` helper with reflection slots and direct calls. The
  fixture checks lookup, declared-method enumeration, modifiers, declaring
  class, parameter/return/exception metadata, invocation, and wrapped exact
  overflow failures. Its complete output matches Temurin 21.0.11.
