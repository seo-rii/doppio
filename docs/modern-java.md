...
- `java.lang.management.ManagementFactory` now covers selected runtime,
  ...
  current-thread CPU/user time, and memory verbose no-op behavior.
- The shim intentionally reports synthetic in-VM counts and empty VM input
  arguments. Detailed `ThreadInfo`, memory usage/pool/manager metadata,
  compilation counters, safepoint/class-size counters, and host JVM diagnostic
