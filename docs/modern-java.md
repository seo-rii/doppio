...
precedence including single-clause init/step return-before-predicate ordering,
plus optional `fini`, covering
...
- `grunt --grunt-ignore-compile-errors test-modern-java` compiles and checks the
  ...
  single-clause init/step return-before-predicate ordering, sealed metadata, and
  illegal direct subtype rejection;
