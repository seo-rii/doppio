...
`MethodHandles.whileLoop`/`MethodHandles.doWhileLoop` non-`void` state-loop
slices through public overlays, covering normal, exceptional, and `void`
`tryFinally` target flows plus stateful loop flows in the Java fixtures and
...
- The minimal `Hello.kt` smoke runs in the `Modern Java` GitHub Actions
  ...
  selected `tryFinally` flows, and selected `whileLoop`/`doWhileLoop`
  non-`void` state loops), and
