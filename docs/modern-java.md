...
selected `MethodHandles.tryFinally` support and the first selected
`MethodHandles.whileLoop` non-`void` state-loop slice through public overlays,
covering normal, exceptional, and `void` `tryFinally` target flows plus
stateful `whileLoop` flows in the Java fixtures and Kotlin smoke. Full
...
- The minimal `Hello.kt` smoke runs in the `Modern Java` GitHub Actions
  ...
  selected `tryFinally` flows, and selected `whileLoop` non-`void` state loops),
  and
...
- `grunt --grunt-ignore-compile-errors test-modern-java` compiles and checks the
  ...
  `asFixedArity` adapter flows, and Java 17 public overlays `zero`, `empty`,
  `arrayLength`, `arrayConstructor`, `dropArgumentsToMatch`, `dropReturn`,
  selected `tryFinally` flows, and selected `whileLoop` non-`void` state loops with
