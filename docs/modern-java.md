...
- `java.lang.management.ManagementFactory` now covers selected runtime,
  class-loading, thread, and memory metadata through the legacy
  ...
  current-thread CPU/user time, memory verbose no-op behavior, and synthetic
  heap/non-heap `MemoryUsage` values with stable field invariants.
- The shim intentionally reports synthetic in-VM counts and empty VM input
  ...
  memory accounting, compilation counters, safepoint/class-size counters, and
  host JVM diagnostic data are not implemented.
