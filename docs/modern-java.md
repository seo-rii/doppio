...
`MethodHandles.tableSwitch` selector/fallback support, plus selected
...
target/fallback dispatch, plus stateful loop flows in the Java fixtures and
Kotlin smoke. Full control-flow combinator parity is still not claimed.
...
- The minimal `Hello.kt` smoke runs in the `Modern Java` GitHub Actions
  ...
  selected `tryFinally` and `tableSwitch` flows, and selected
